...
relationshipsRoute.register = () => {
  router.bindRoute({
    ...
    try {
      ...
      const resource = await Promisify.promisifySingle(resourceConfig.handlers.find.bind(resourceConfig.handlers))(request)

      postProcess.fetchForeignKeys(resource, resourceConfig.attributes)

      const sanitisedData = await responseHelper._checkSchemaOnObject(resource, resourceConfig.attributes)

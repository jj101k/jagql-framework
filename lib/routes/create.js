...
createRoute.register = () => {
  router.bindRoute({
    ...
    try {
      ...
      const createdResource = await Promisify.promisifySingle(resourceConfig.handlers.create.bind(resourceConfig.handlers))(request, theirResource)

      request.params.id = '' + createdResource.id
      const newResource = await Promisify.promisifySingle(resourceConfig.handlers.find.bind(resourceConfig))(request)

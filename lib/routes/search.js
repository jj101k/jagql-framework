...
export class searchRoute {
    ...
    static register(router, configStore, responseHelper, relationshipStore) {
        router.bindRoute({
            ...
            const schemaChecker = new ArraySchemaChecker(resourceConfig, responseHelper)
            const dataItemInfo = new DataItemInfo(resourceConfig, relationshipStore)
             ...
             */
            let paginationInfo
            const filterF = FilterHelper.getHelper(handler, request, resourceConfig, relationshipStore)
            ...
            if("next" in r) {
                sanitisedData = []
                paginationInfo = 0
                const relStripper = new RemoteRelationshipStripper(resourceConfig, relationshipStore)
                ...
                postProcess.stripRemoteRelationships(searchResults, resourceConfig, relationshipStore)
            ...
            response.included = []
            await postProcess.handleMulti(router, configStore, request, response, relationshipStore)

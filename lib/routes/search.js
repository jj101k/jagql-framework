    ...
    static register() {
        router.bindRoute({
            ...
            if(handler.handlesFilter || !request.processedFilter) {
                ...
                const relationships = new Set(Relationship.getAllRelationships(resourceConfig).map(([r]) => r))
                const pf = request.processedFilter
                filterF = r => r.filter(ri => FilterHelper.filterKeepInternalObject(ri, pf, relationships))
            }
            ...
            if("next" in r) {
                ...
                for await (const [sr, pi] of r) {
                    const filtered = filterF(sr)
                    const page = paginationEnforcer.enforce(filtered)
                    relStripper.strip(page)
                    sanitisedData.push(...await responseHelper.generateDataItems(page, dataItemInfo))
                    paginationInfo += pi + filtered.length - sr.length
                ...
                const filtered = filterF(searchResultsIn)
                paginationInfo -= searchResultsIn.length - filtered.length
                const searchResults = paginationEnforcer.enforce(filtered)

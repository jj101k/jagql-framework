...
class ourJoi {
  ...
  static Joi = {
     ...
     * @returns
     */
    manyOf(resourceOrResources, uidType) {
      const resources = tools.ensureArray(resourceOrResources)
      for(const resource of resources) {
        if (typeof resource !== 'string') throw new Error('Expected a string when defining a primary relation via .manyOf()')
      }
      ...
      return this._ensureSettings(obj, (settings) => {
        ...
        settings._uidType = uidType ?? "string"
      })
    },
    /**
     *
     ...
     * @returns
     */
    oneOf(resourceOrResources, uidType) {
      const resources = tools.ensureArray(resourceOrResources)
      for(const resource of resources) {
        if (typeof resource !== 'string') throw new Error('Expected a string when defining a primary relation via .oneOf()')
      }

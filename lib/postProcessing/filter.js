...
module.exports = class filter {
   ...
   * @param {import('../../types/Handler.js').FilterSpec[]} allowList
   * @returns
   */
  static #relationshipMatchesOR(relationships, allowList) {
    const relOrRels = relationships.data
    if (!relOrRels) return false
    const allowListCheck = new Set(allowList.map(a => a.value))

    const rels = tools.ensureArray(relOrRels)

    const relIds = rels.map(relation => relation.id)

    return relIds.some(id => allowListCheck.has(id))
  }
}
...
module.exports = class includePP {
   ...
   */
  static async #fillIncludeTree(includeTree, request, callback) {
    /** **
    includeTree = {
      _dataItems: [ ],
      _filter: { },
    ...
    ****/
    ...
    for(const relation of Object.keys(relationsToInclude.primary)) {
      const ids = [...new Set(relationsToInclude.primary[relation])]
      const parts = relation.split('~~')
      let query = ids.map(id => `filter[id]=${id}`).join("&")
      if (includeTree[parts[1]]._filter) {
        query += `&${includeTree[parts[1]]._filter.join('&')}`
      }
      resourcesToFetch.push({
        url: `${jsonApi._apiConfig.base + parts[0]}/?${query}`,
        as: relation
      })
    }

    for(const relation of Object.keys(relationsToInclude.foreign)) {
      const ids = [...new Set(relationsToInclude.foreign[relation])]
      ids.sort((a, b) => a.localeCompare(b))
      const parts = relation.split('~~')
      let query = ids.map(id => `filter[${parts[0]}]=${id}`).join("&")
      if (includeTree[parts[2]]._filter) {
        query += `&${includeTree[parts[2]]._filter.join('&')}`
      }
      resourcesToFetch.push({
        url: `${jsonApi._apiConfig.base + parts[1]}/?${query}`,
        as: relation
      })
    }
    ...
    try {
      ...
      for(const include of includes) {
        ...
        await new Promise((resolve, reject) => this.#fillIncludeTree(includeTree[include], request, err => err ? reject(err) : resolve()))
      }
    } catch(err) {
      return callback(err)
    }

...
export class rerouter {
     ...
     * import("./JsonApiRequest.js").JsonApiRequest, query: import("./JsonApiRequest.js").JsonApiQueryParams}} newRequest
     */
    static route(router, newRequest) {
        const validRoutes = router.routes[newRequest.method.toLowerCase()]

        const path = this.#generateSanePath(router, newRequest)
         ...
         * "params" | "query" | "url"> & {originalUrl: string, body: any}}
         */
        const req = {
     ...
     * "foo/bar"
     */
    static #generateSanePath(router, newRequest) {
        // Note: URI might be partial, so set a base.
        const uri = new URL(newRequest.uri, "https://www.example.org/")

        return router.stripBasePath(uri.pathname).replace(/^\//, "").replace(/\/$/, "")
    }

    /**
     *
     * @param {import("./Router.js").Router["routes"][""]} validRoutes
     * @param {string} path
     * @returns
     */
    static #pickFirstMatchingRoute(validRoutes, path) {
        return Object.keys(validRoutes).filter(someRoute => {
            const routePattern = someRoute.replace(/(:[a-z]+)/g, "[^/]*?")
            return path.match(new RegExp(`^${routePattern}$`))
        }).pop()
    }

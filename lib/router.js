...
module.exports = class router {
  ...
  static #getUrlComponents(reqUrl) {
    if (jsonApi._apiConfig.urlPrefixAlias) {
      return {base: jsonApi._apiConfig.urlPrefixAlias.replace(/\/$/, ''),
        reqUrl: reqUrl.replace(jsonApi._apiConfig.base, '/')}
    } else {
      return {base: urlTools.concatenateUrlPrefix(jsonApi._apiConfig),
        reqUrl}
    }
  }
   ...
   *
   * @param {*} config
   * @param {<R>(req: import('../types/Handler.js').JsonApiRequest, resourceConfig: import("../types/ResourceConfig.js").ResourceConfig<R>, res: any) => *} callback
   */
   ...
   *
   * @param {*} req
   * @returns {import('../types/Handler.js').JsonApiRequest}
   */

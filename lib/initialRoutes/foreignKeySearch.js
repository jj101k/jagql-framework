    ...
    static register(router, configStore, responseHelper, relationshipStore) {
        ...
        router.bindRoute({
            ...
            try {
                ...
                if (!Relationship.getRelationship(resourceConfig, foreignKey, relationshipStore)) {
                    throw new JsonApiError({
                        status: 403,
                        code: "EFORBIDDEN",
                        title: "Invalid foreign key lookup",
                ...
                return helper.handleError(router, request, res, err, responseHelper)
            }

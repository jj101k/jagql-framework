...
swaggerPaths._getResourceDefinition = resourceConfig => {
  ...
  for (const attribute in attributes) {
    ...
    const attrSchema = attributes[attribute]

    let swaggerScheme = { }
    if (attrSchema._description) {
      swaggerScheme.description = attrSchema._description
    }

    if (!attrSchema._settings) {
      swaggerScheme.type = attrSchema._type
      if (swaggerScheme.type === 'date') {
        swaggerScheme.type = 'string'
        swaggerScheme.format = 'date'
      }
      if (swaggerScheme.type === 'array') {
        const items = attrSchema._inner.items
      ...
      if (attrSchema._flags?.presence === "required") {
      ...
      if (attrSchema._settings.as) continue
      ...
      if (attrSchema._settings.__many) {

...
module.exports = class swaggerResources {
    ...
    for (const [attribute, attrSchema] of Object.entries(attributes)) {
      ...
      if (settings) {
        ...
        relationshipsShortcut[attribute] =
          swaggerResources.#getRelationshipSchema(attrSchema, settings)
      } else {
   ...
   *
   * @param {import('joi').Schema} attrSchema
   * @param {import("../../types/ourJoi").OurJoiSettings} settings
   * @returns
   */
  static #getRelationshipSchema(attrSchema, settings) {
    const describe = attrSchema.describe()
    const itemSchema = {
      type: 'object',
      required: ['type', 'id'],

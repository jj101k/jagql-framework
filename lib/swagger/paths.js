...
swaggerPaths._addPathDefinition = (paths, resourceConfig) => {
  ...
  Object.keys(resourceConfig.actions).forEach(actionName => {
    swaggerPaths._addActionPath(paths, resourceName, actionName, resourceConfig.actions[actionName])
  })
}

swaggerPaths._addActionPath = (paths, resourceName, actionName, action) => {
  const actionPaths = { }
  const actionConfig = action._settings._action
  paths[`/${resourceName}/{id}/${actionName}`] = actionPaths
  if (actionConfig.get) {
    actionPaths.get = swaggerPaths._getActionObject({
      resourceName,
  ...
  if (actionConfig.post) {
    actionPaths.post = swaggerPaths._getActionObject({
      resourceName,
...
/**
 *
 * @param {{parameters: Record<string, import('joi').Schema>}} options
 * @returns
 */
swaggerPaths._getActionObject = options => {
  ...
  if (options.parameters) {
    const additionalParams = Object.keys(options.parameters).map(paramName => {
      ...
      return {
        name: paramName,
        in: 'query',
        description: optSchema._description || undefined,
        required: optSchema._flags?.presence === "required",
        type: optSchema._type
      }
    })

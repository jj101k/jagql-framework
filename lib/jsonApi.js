...
const concatenateUrlPrefix = (config) => {
...
jsonApi.setConfig = function(apiConfig) {
  ...
  this._apiConfig.pathPrefix = apiConfig.urlPrefixAlias || concatenateUrlPrefix(this._apiConfig)
...
jsonApi.define = function(resourceConfig, options) {
  ...
  resourceConfig.onCreate = _.pick.apply(
    _,
    [].concat(
      resourceConfig.attributes,

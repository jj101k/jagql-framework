 ...
 * @returns
 */
jsonApi._concatenateUrlPrefix = config => {
 ...
 */
jsonApi.define = (resourceConfig, options) => {
  ...
  resourceConfig.onCreate = _.pick.apply(_, [].concat(resourceConfig.attributes, Object.keys(resourceConfig.attributes).filter(i => !resourceConfig.attributes[i]._meta.includes("readonly") && !resourceConfig.attributes[i]._settings?.__as)))
}

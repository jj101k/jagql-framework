...
/**
 * @type {express.Express | express.Router}
 */
let app
/**
...
export class Router {
    ...
    async #authenticate(request, res) {
        ...
        try {
            ...
            console.warn(err)
            const payload = this.#responseHelper.generateError(request, new JsonApiError({
                status: 401,
                code: "UNAUTHORIZED",
     ...
     * @param {Record<string,
     * import("./ResourceConfig.js").ResourceConfig<any>>} resources
     * @param {import("./responseHelper.js").responseHelper} responseHelper
     * @param {import("./metrics.js").metrics} metrics
     */
    constructor(configStore, resources, responseHelper, metrics) {
     ...
     *
     * @param {{verb: import("./JsonApiRequest.js").HttpVerbs, path: string}} config
     * @param {<R>(req: JsonApiRequest, resourceConfig: import("./ResourceConfig.js").ResourceConfig<R>, res: import("express").Response) => *} handler
     */
    bindRoute(config, handler) {
        ...
        const routeHandler = async (req, res, next) => {
            ...
            const request = this.#getParams(req, res)
            const resourceConfig = this.#resources[request.routeParams.type]
    ...
    sendResponse(res, payload, httpCode) {
        ...
        this.#metrics.processResponse(res.locals.jsonApi?.request, httpCode, payload, timeDiff)

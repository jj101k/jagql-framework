...
module.exports = class postProcess {
  ...
  static async handle(request, response) {
    ...
    await this.#postProcess(request, response, "fields", (request, response) => fields.action(request, response))
  }
  ...
  static fetchForeignKeys(itemOrItems, resourceConfig) {
    ...
    const stripKeys = Relation.getAllRelations(resourceConfig).filter(([i, rel]) => {
      return (rel instanceof RemoteRelation) ? rel.remoteKey : undefined
    }).map(([i]) => i)
    if(!stripKeys.length) {
      return
    }

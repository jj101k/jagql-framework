...
module.exports = class filter {
  ...
  static #relationshipIsForeign(resourceConfig, key) {
    const relationSettings = ourJoi.getAttributeSettings(resourceConfig, key)
  ...
  static #parseFilterDef(attributeName, resourceConfig, scalarElement) {
    ...
    const error = this.#stringOnlyOperator(splitElement.operator,
      resourceConfig.attributes[attributeName])
    if (error) return { error }

    if (ourJoi.getAttributeSettings(resourceConfig, attributeName)) { // relationship attribute: no further validation
      return { result: splitElement }
    }

    const schema = resourceConfig.attributes[attributeName]

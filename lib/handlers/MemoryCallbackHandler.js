...
class MemoryCallbackHandler extends Handler {
  ...
  search (request, callback) {
    ...
    return callback(null, MemoryCallbackHandler.#clone(resultsPage), resultCount)
  }
  ...
  delete (request, callback) {
    ...
    this.find(request, (err, theResource) => {
      ...
      const index = MemoryCallbackHandler.#indexOf(this.resources[request.params.type], theResource)
  ...
  update (request, partialResource, callback) {
    ...
    this.find(request, (err, theResource) => {
      ...
      theResource = {...theResource, ...partialResource}
      ...
      return callback(null, MemoryCallbackHandler.#clone(theResource))
    })
  }
  ...
  create (request, newResource, callback) {
    ...
    const index = MemoryCallbackHandler.#indexOf(this.resources[request.params.type], newResource)
    if (index !== -1) {
      return callback({ // eslint-disable-line standard/no-callback-literal
        status: '403',
        code: 'EFORBIDDEN',
    ...
    return callback(null, MemoryCallbackHandler.#clone(newResource))
  }
}

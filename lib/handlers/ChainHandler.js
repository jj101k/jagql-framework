  ...
  ChainHandler.prototype[lowerAction] = async function (...argsIn) {
    ...
    try {
      const argsBefore = this[`before${action}`] ?
        await Promisify.promisifyFull(this[`before${action}`].bind(this))(...argsIn) : argsIn
      const result = await Promisify.promisifyFull(this.otherHandler[lowerAction].bind(this.otherHandler))(...argsBefore)

      after = this[`after${action}`] ?
        await Promisify.promisifyFull(this[`after${action}`]).bind(this)(request, ...result) :
        result
    } catch(err) {
      return callback(err)
    }

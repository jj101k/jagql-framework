  ...
  ChainHandler.prototype[lowerAction] = async function (...argsIn) {
    ...
    try {
      if(this[`before${action}`]) {
        argsOut = await Promisify.promisifyFull(this[`before${action}`].bind(this))(...argsOut)
      }
      argsOut = await Promisify.promisifyFull(this.otherHandler[lowerAction].bind(this.otherHandler))(...argsOut)

      if(this[`after${action}`]) {
        argsOut = await Promisify.promisifyFull(this[`after${action}`]).bind(this)(request, ...argsOut)
      }
    } catch(err) {
      return callback(err)
    }
    callback(null, ...argsOut)
  }
})

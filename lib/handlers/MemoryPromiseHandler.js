...
class MemoryPromiseHandler extends Handler {
    ...
    create(request, newResource) {
        ...
        if (index !== -1) {
            throw new JsonApiError({
                ...
                detail: `The requested resource already exists of type ${type} with id ${request.body.id}`
            })
        }
    ...
    async *search(request) {
        ...
        const resultsPage = this.#paginateResultSet(results, request.appParams.page)
        yield [[], resultCount]

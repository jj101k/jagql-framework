...
responseHelper._checkSchemaOnArray = (items, schema, callback) => {
  let checkItems
  ...
  async.mapSeries(checkItems,
    (item, done) => responseHelper._enforceSchemaOnObject(item, schema, done),
    () => {
      // Whatever happens, we return the success response
      callback(null, items.map(sanitisedItem => responseHelper._generateDataItem(
              sanitisedItem, schema)))
    }
  )
}

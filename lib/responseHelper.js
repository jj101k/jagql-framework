...
export class responseHelper {
     ...
     * @returns
     */
    #getLinkProperties(resourceConfig) {
        return Relationship.getAllRelationships(resourceConfig, this.#relationshipStore).map(([k]) => k)
    }
     ...
     * @returns
     */
    async checkSchemaOnObject(cItem, resourceConfig, inferredBaseUrl) {
        ...
        const dataItemInfo = new DataItemInfo(resourceConfig, this.#relationshipStore)
     ...
     * @returns
     */
    async enforceSchemaOnObject(cItem, resourceConfig, inferredBaseUrl) {
        const baseUrl = inferredBaseUrl ?? this.#baseUrl
        debug.validationOutput(JSON.stringify(cItem))
        const item = await tools.loadPromises(cItem, resourceConfig)
        this.convertId(item, resourceConfig)
     ...
     * @returns
     */
    async #generateDataItem(item, dataItemInfo, baseUrl) {
     ...
     * @returns {import("./JsonApiResponse.js").JsonApiResourceObject[] | Promise<import("./JsonApiResponse.js").JsonApiResourceObject[]>}
     */
    generateDataItems(items, dataItemInfo, baseUrl) {
        if(!items.length) {
            return []
        }
        const { attributeProperties, linkProperties, relationships: relationshipsIn } = dataItemInfo
        ...
        /**
         * @type {import("./JsonApiResponse.js").JsonApiResourceObject[]}
         */
        const results = []
        const promises = []
     ...
     * @template {{id: string, type: string, meta?: *}} R
     * @param {R} item
     * @param {Record<string, BaseRelationship>} relationshipsIn
     * @param {*} linkProperties
     * @param {string} baseUrl
     * @returns
     */
    #generateRelationships(item, relationshipsIn, linkProperties, baseUrl) {
        if (linkProperties.length === 0) return undefined

        /**
         * @type {Record<string, import("./JsonApiResponse.js").JsonApiLink>}
         */
        const links = { }
     ...
     * @param {string} baseUrl
     * @returns {Promise<import("./JsonApiResponse.js").JsonApiLink>}
     */
    async #generateLinkRemote(item, linkProperty, rel, baseUrl) {
        const relatedResource = rel.resourceNames[0]
        return {
            meta: {
                relation: "foreign",
     ...
     * @returns
     */
    async #generateLinkOneInner(item, linkProperty, linkItem, baseUrl) {
        return {
            meta: {
                /**
                 * @type {"primary"}
     ...
     * @returns
     */
    async #generateLinkOne(item, linkProperty, baseUrl) {
     ...
     * @returns {Promise<import("./JsonApiResponse.js").JsonApiLink>}
     */
    async #generateLinkManyInner(item, linkProperty, linkItemOrItems, baseUrl) {
        return {
            meta: {
                relation: "primary",
                // type: rel.resources,
     ...
     * @returns {Promise<import("./JsonApiResponse.js").JsonApiLink>}
     */
    async #generateLinkMany(item, linkProperty, baseUrl) {
        if(tools.isPromise(item[linkProperty])) {
     ...
     *
     * @param {*} item
     * @param {BaseRelationship} rel
     * @param {*} linkProperty
     * @param {string} baseUrl
     * @returns {import("./JsonApiResponse.js").JsonApiLink | Promise<import("./JsonApiResponse.js").JsonApiLink>}
     */
    #generateLink(item, rel, linkProperty, baseUrl) {
    ...
    /**
     *
     * @param {import("./JsonApiResponse.js").JsonApiResponseError} error
     */
    #sanitiseError(error) {
        /**
         *
         ...
         *
         * @param {*} v
         * @returns {import("./JsonApiResponse.js").JsonApiLinkObject |
         * string | null}
         */
        const _linkObject = (v) => {
            if(v === null) {
     ...
     * @returns {import("./JsonApiResponse.js").JsonApiResponseBodyErrorWithMeta}
     */
    generateError(request, errOrErrs) {
     ...
     * @param {*} sanitisedData
     * @param {*} handlerTotal
     * @returns {import("./JsonApiResponse.js").JsonApiResponseBodyWithMeta}
     */
    generateResponse(request, resourceConfig, sanitisedData, handlerTotal) {
        const baseUrl = request.inferredBaseUrl ?? this.#baseUrl
        return {
            jsonapi: {
                version: "1.0"
     ...
     * @returns
     */
    generateMeta(request, handlerTotal) {

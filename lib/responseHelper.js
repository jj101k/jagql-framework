...
module.exports = class responseHelper {
   ...
   */
  static async _checkSchemaOnArray(itemOrItems, schema) {
    const checkItems = Array.isArray(itemOrItems) ?
      itemOrItems.slice(0, 10) : // Only check the first 10 items, for basic sanity checks
      [itemOrItems]
  ...
  static async #generateLink(item, schemaProperty, linkProperty) {
    ...
    if (settings.__many && item[linkProperty] !== undefined) {
      ...
      if (linkItemOrItems) {
        const linkItems = Array.isArray(linkItemOrItems) ? linkItemOrItems : [linkItemOrItems]

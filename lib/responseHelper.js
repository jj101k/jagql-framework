...
function convertId (item, schema) {
  ...
  for(const p of getLinkProperties(schema)) {
    ...
    if(!v) continue
    if(Array.isArray(v)) {
      item[p] = v.map(vi => ({id: '' + vi.id, type: vi.type}))
    } else {
      item[p] = {id: '' + v.id, type: v.type}
    }
  }
}
...
responseHelper._checkSchemaOnObject = async (cItem, schema, callback) => {
...
responseHelper._enforceSchemaOnObject = async (cItem, schema, callback) => {
  debug.validationOutput(JSON.stringify(cItem))
  const item = await loadPromises(cItem, schema)
  convertId(item, schema)
  Joi.validate(item, schema, async (err, sanitisedItem) => {
...
responseHelper._isPromise = isPromise
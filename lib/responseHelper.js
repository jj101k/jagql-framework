...
module.exports = class responseHelper {
   ...
   * @param schema
   */
  static #convertId(item, schema) {
    if (item.id) {
      item.id = '' + item.id
    }
    for(const p of this.#getLinkProperties(schema)) {
      const v = item[p]
      if(!v) continue
      if(Array.isArray(v)) {
   ...
   * @returns
   */
  static #getLinkProperties(schema) {
    return Object.keys(schema).filter(someProperty => this.#isSpecialProperty(schema[someProperty]))
  }
   ...
   * @returns
   */
  static #isSpecialProperty(value) {
   ...
   * @returns
   */
  static async checkSchemaOnObject(cItem, schema) {
   ...
   * @returns
   */
  static async enforceSchemaOnObject(cItem, schema) {
    debug.validationOutput(JSON.stringify(cItem))
    const item = await loadPromises(cItem, schema)
    this.#convertId(item, schema)
  ...
  static async #generateDataItem(item, schema) {
    const linkProperties = this.#getLinkProperties(schema)
    const attributeProperties = Object.keys(schema).filter(someProperty => {
      ...
      return !this.#isSpecialProperty(schema[someProperty])
    })

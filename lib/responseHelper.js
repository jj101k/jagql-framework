...
module.exports = class responseHelper {
  ...
  static #getLinkProperties(resourceConfig) {
    return Object.keys(resourceConfig.attributes).filter(
      someProperty => this.#isSpecialProperty(resourceConfig, someProperty))
  }

  /**
   * Called a few times for every datum produced by a request
   *
   * @param {import('../types/ResourceConfig').ResourceConfig} resourceConfig
   * @param {string} key
   * @returns
   */
  static #isSpecialProperty(resourceConfig, key) {
    if (!(resourceConfig.attributes[key] instanceof Object)) return false
    if (ourJoi.getAttributeSettings(resourceConfig, key)) return true
    return false
  }
  ...
  static async checkSchemaOnArray(items, resourceConfig) {
    ...
    const result = []
    const compiledSchema = Joi.compile(resourceConfig.attributes)
    for (const cItem of checkItems) {
      const item = await loadPromises(cItem, resourceConfig.attributes)
    ...
    for (const item of items) {
      result.push(await this.#generateDataItem(item, resourceConfig))
    }
    return result
  }
  ...
  static async enforceSchemaOnObject(cItem, resourceConfig) {
    ...
    const item = await loadPromises(cItem, resourceConfig.attributes)
    this.#convertId(item, resourceConfig)
    const compiled = Joi.compile(resourceConfig.attributes)
  ...
  static async #generateDataItem(item, resourceConfig) {
    ...
    const attributeProperties = Object.keys(resourceConfig.attributes).filter(someProperty => {
      ...
      return !this.#isSpecialProperty(resourceConfig, someProperty)
    })
    ...
    const result = {
      ...
      id: '' + item.id,
      attributes,
      links: this.#generateLinks(item, resourceConfig.attributes, linkProperties),
  ...
  static async #generateLink(item, resourceConfig, linkProperty) {
    const settings = ourJoi.getAttributeSettings(resourceConfig, linkProperty)
    const link = {
      meta: {
        relation: 'primary',

...
module.exports = class responseHelper {
  ...
  static async #generateDataItem(item, dataItemInfo) {
    const { attributeProperties, linkProperties, relations } = dataItemInfo

    const attributes = Object.fromEntries(
      attributeProperties.filter(p => p in item).map(p => [p, item[p]]))

    const relationships = this.#generateRelationships(item, relations, linkProperties)
   ...
   * @template {{id: string, type: string, meta?: *}} R
   * @param {R} item
   * @param {Record<string, import("./BaseRelation.js")>} relations
   * @param {*} linkProperties
   * @returns
   */
  static #generateRelationships(item, relations, linkProperties) {
    ...
    for (const linkProperty of linkProperties) {
      const rel = relations[linkProperty]
   ...
   *
   * @param {*} item
   * @param {import("./BaseRelation.js")} rel
   * @param {*} linkProperty
